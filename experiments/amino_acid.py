...
class AminoAcid():
    ...
    def _init_atom_group_pe(self):
        ...
        self.pe, self.permutation = autils.get_atom_groups(
            self.mol, self.exist_atoms)
        if self.has_conf:
    ...
    def _init_gt_positions(self):
        ...
        match_pattern = self.mol.GetSubstructMatches(
        ...
        if len(match_pattern) > 0:
            ...
            self._map_from_inner_idx_to_output_idx[match_pattern[0]
                                                   [-1]] = self.non_H_atoms-1
        ...
        for i in range(self.non_H_atoms):
            ...
            if inner_idx not in self._map_from_inner_idx_to_output_idx:
                self._map_from_inner_idx_to_output_idx[inner_idx] = len(
                    self._map_from_inner_idx_to_output_idx)
        ...
        assert self._out_pe.min() > - \
            1, f"encounter error in check out pe {self._out_pe}"
        # init edge feats
        ...
        self._out_default_positions = self._out_default_positions - \
            self._out_default_positions.mean(0, keepdims=True)

        # aug with random rotation
        random_rot = scipy.spatial.transform.Rotation.random()
        self._out_default_positions = random_rot.apply(
            self._out_default_positions)
