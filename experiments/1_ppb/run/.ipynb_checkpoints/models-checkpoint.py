...
class RepeatedModule(nn.Module):
    ...
    def forward(self, peptide_sequence, nodes, edges, neighbor_indices, bert_features, pro_chaininfo):
        ...
        encoded_nodes = self.node_embedding(torch.cat([nodes, bert_features], dim=-1))
        ...
        for reciprocal_layer in self.reciprocal_layer_stack:
            ...
            node_enc, sequence_enc, graph_attention, sequence_attention, node_seq_attention, seq_node_attention =\
                reciprocal_layer(sequence_enc, node_enc, edge_input, pro_chaininfo)
...
class FullModel(nn.Module):
    ...
    def forward(self, batch):
        ...
        pep_residue_index=peptide.residue_index[0]
        peptide_sequence = self.peptide_model(peptide.x, peptide.edge_index, peptide.edge_attr)#[None]
        ...
        graph_seq_attention=torch.cat(graph_seq_attention_list, 1).squeeze(0).transpose(-1, -2)
        pair_attn=torch.cat([seq_graph_attention, graph_seq_attention], 0).permute(1,2,0)
        ...
        merge_shape=[sequence_enc.shape[0],node_enc.shape[0],  node_enc.shape[-1]]
        pred=self.out_layer(pair_attn).squeeze(-1)#torch.cat([node_enc[None].expand(merge_shape), sequence_enc[:,None].expand(merge_shape)], -1)).squeeze(-1)
