...
from protein.graph import mmcif_to_prograph
    ...
    for i in sorted(valid_res.keys()):
        ...
        if res_flag.position:  # residues without missing
            ...
            res_graph, res_mol = from_smiles(
                f"raw_data/vocab/{res_name}_ideal.sdf"
            )
...
def single_data_preprocess(data, raw_dir, processed_dir, args, update_peptide_only=False, update_protein_only=False, cutoffs=[2.5, 3, 4, 5, 6], gt_only=False, pretrained_sincaa=None):
    """
    ...
    gt_only = (gt_only or (osp.exists(osp.join(data_path, f"{data}_peptide_info{peptide_endfix}.pt")) and osp.exists(
        osp.join(data_path, f"{data}_protein_info.pt")))) and not update_peptide_only and not update_protein_only
    if gt_only:
        print(data)
        peptide_info = torch.load(osp.join(
            data_path, f"{data}_peptide_info{peptide_endfix}.pt"), weights_only=False)
        # protein_graph=torch.load(osp.join(data_path, f"{data}_protein_graph.pt"), weights_only=False)
        protein_info = torch.load(
            osp.join(data_path, f"{data}_protein_info.pt"), weights_only=False)
    else:
        if update_peptide_only:
            if os.path.exists(osp.join(data_path, f"{data}_peptide_info{peptide_endfix}.pt")):
                os.remove(
                    osp.join(data_path, f"{data}_peptide_info{peptide_endfix}.pt"))
            peptide_info = mmcif_to_graph(
                cif_file, data, chain, args.emb_path, pretrained_sincaa=pretrained_sincaa)

            torch.save(peptide_info, osp.join(
                data_path, f"{data}_peptide_info{peptide_endfix}.pt"))
            return
        protein_info = mmcif_to_prograph(
            args, cif_file, data, target, data_path)
        # torch.save(protein_graph, osp.join(data_path, f"{data}_protein_graph.pt"))
        torch.save(protein_info, osp.join(
            data_path, f"{data}_protein_info.pt"))
        if update_protein_only:
            return
        peptide_info = torch.load(osp.join(
            data_path, f"{data}_peptide_info{peptide_endfix}.pt"), weights_only=False)
    ...
    protein_position = protein_info.x
    ...
    total_res_num = sum(
        [len(_[1]) for k, _ in protein_info.map_from_chain_id_to_seq.items()])
    min_dist_between_peptide_and_pro_res = np.zeros(
        [len(peptide_position), total_res_num])-1
    row_info = []
    shift = 0
    sort_keys = sorted(protein_info.map_from_chain_id_to_seq.keys())
    for cid in sort_keys:
        s = protein_info.map_from_chain_id_to_seq[cid]
        pos = (np.array(chain_ids) == cid)
        unique_r_ids = np.unique(residue_index[pos])
        s = s[1]
        for rid in unique_r_ids:
            t = distance_between_peptide_and_protein[:, (pos & (
                residue_index == rid))].min(1)
            min_dist_between_peptide_and_pro_res[:, rid+shift] = t
        shift = shift+len(s)
        row_info = row_info+[(cid, i) for i in range(len(s))]
    print("writing", osp.join(data_path, f"{data}_residue_label.pt"))
    torch.save(Data(x=min_dist_between_peptide_and_pro_res,
               row_info=row_info), osp.join(data_path, f"{data}_residue_label.pt"))
