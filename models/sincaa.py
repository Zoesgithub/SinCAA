...
class SinCAA(nn.Module):
    def __init__(self, args) -> None:
        ...
        if hasattr(args, "model") and args.model == "GAT":
            ...
            self.topological_net = construct_gps(args.topological_net_layers, args.model_channels,
                                                 num_head=args.num_head, attn_type="multihead", norm=args.norm, num_inner_l=args.num_inner_l)
            self.model = "GPS"
        ...
        if args.norm == 'BatchNorm':
            self.recovery_info = nn.Sequential(nn.BatchNorm1d(
                args.model_channels), nn.Linear(args.model_channels, 200))
            self.edge_recovery_info = nn.Sequential(nn.BatchNorm1d(
                args.model_channels), nn.Linear(args.model_channels, 200))
        else:
    ...
    def get_num_params(self):
        ...
        topological_net = sum(p.numel()
                              for p in self.topological_net.parameters())
    ...
    def calculate_topol_emb(self, feats, add_mask=False):
        ...
        # gps forward
        x = node_emb
        mask, edge_mask = self.generate_mask(
            x, edge_index, batch_id, add_mask=add_mask)

        x = x*mask
        inpx = x
        assert edge_index.max() == x.shape[0] - \
            1, f"{edge_index.shape} {x.shape}"
        assert edge_index.shape[-1] == 0 or edge_index.max() < len(
            x), f"{edge_index.max} {x.shape}"
        xs = []
        recovery_info_loss = 0

        if self.model == "GAT":
            x = self.topological_net(
                x, edge_index,  edge_attr=edge_emb, batch=batch_id)

        else:
            for i, conv in enumerate(self.topological_net):
                assert len(x) == len(batch_id), f"{x.shape}{batch_id.shape}"
                assert len(
                    edge_emb) == edge_index.shape[-1], f"{edge_emb.shape}{edge_index.shape}"
                if i == 0:
        ...
        for i in range(num_round):
            ...
            recovery_info = self.recovery_info(
                tx[mask.squeeze(-1) < 1]).reshape(-1, 2, 100).reshape(-1, 100)
            l = feats["nodes_int_feats"][mask.squeeze(-1)
                                         < 1][..., :2].reshape(-1)
            recovery_info_loss = recovery_info_loss+(nn.functional.cross_entropy(
                recovery_info, l, reduction='none')).sum()/max(recovery_info.shape[0], 1)
            edge_recover_info = self.edge_recovery_info(
                (tx[edge_index[0]]+tx[edge_index[1]])[edge_mask.squeeze(-1) < 1]).reshape(-1, 2, 100).reshape(-1, 100)
            edge_l = edge_feats[edge_mask.squeeze(-1) < 1].reshape(-1)
            recovery_info_loss = recovery_info_loss+(nn.functional.cross_entropy(
                edge_recover_info, edge_l, reduction='none')).sum()/max(edge_recover_info.shape[0], 1)
